...
public class ArmIOTalonFX implements ArmIO {
  ...
  public ArmIOTalonFX(
      ...
      boolean invertPivot,
      boolean invertExtesion,
      boolean brake,
      double reductionPivot,
      double reductionExtension) {
    ...
    BaseStatusSignal.setUpdateFrequencyForAll(
        50.0, position, velocity, appliedVoltage, supplyCurrentAmps, torqueCurrent, supplyVoltage);
    armPivot.optimizeBusUtilization(0, 1.0);

    // // Extension Config
    // TalonFXConfiguration configExtension = new TalonFXConfiguration();
    // configExtension.MotorOutput.Inverted = invertExtesion ? InvertedValue.Clockwise_Positive :
    // InvertedValue.CounterClockwise_Positive;

    // configExtension.MotorOutput.NeutralMode = brake ? NeutralModeValue.Brake :
